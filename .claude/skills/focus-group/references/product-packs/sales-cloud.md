...
| **Reporting Snapshots vs. Tableau / CRM Analytics** | Historical pipeline / trending in Sales Cloud requires either Reporting Snapshots (configured) or a separate analytics product — confirm before promising trended pipeline reporting. The previously-cited slug `sf.data_concepts_reporting_snapshots.htm` 404s; verified canonical slug not located in 4 URL guesses (`sf.reporting_snapshot.htm`, `sf.reports_snapshots.htm`, `sf.create_a_reporting_snapshot.htm`, `sf.creating_reporting_snapshots.htm` all 404) — search engines were captcha-blocked. | https://help.salesforce.com/s/articleView?id=sf.data_concepts_reporting_snapshots.htm | TODO — canonical Reporting Snapshots help slug not located; navigate from analytics root in the help TOC |

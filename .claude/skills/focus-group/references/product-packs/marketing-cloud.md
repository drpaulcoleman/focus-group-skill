...
| **Data Cloud (Data 360) + Marketing Cloud integration** | Data Cloud audiences can be activated in Marketing Cloud Growth/Advanced; legacy SFMC integration is via the Salesforce Audience connector with specific patterns and limits. The previously-cited slug `sf.c360_a_marketing_cloud_audiences.htm` 404s; per the Forecasting doc verified in this pass: "As of October 14, 2025, Data Cloud has been rebranded to Data 360." Re-attempted with real Chrome 2026-05-22 against the help.salesforce.com SPA: returns 461KB shell with generic title but no article body, so candidate slugs in `data360_*` and `data_cloud_*` namespaces cannot be tested via this harvester. | https://help.salesforce.com/s/articleView?id=sf.forecasts3_intro.htm | TODO — Data 360 rebrand verified; activation slug must be located via in-browser navigation (help SPA does not render body in headless capture) |
| **API limits — Marketing Cloud REST and SOAP** | Re-attempted with real Chrome 2026-05-22; the developer-guide index sidebar (captured 454KB) confirms a canonical "API Performance" slug (`/docs/marketing/marketing-cloud/guide/api-performance.html`) and an "APIs Overview" slug (`apis-overview.html`); per-edition rate-limit numbers themselves were not extracted from the captured body, but the index sidebar verifies the canonical reference page exists. Quote specific rate limits only against the customer's installed-package config (Setup → Apps → Installed Packages) or `api-performance.html` opened in a real browser. | https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/api-performance.html | TODO — slug verified in dev-guide sidebar; per-edition rate-limit numbers require browser navigation to the page body (LWR doc shell does not render body in headless capture) |
| **AMPscript / SSJS — execution context** | Re-attempted with real Chrome 2026-05-22; dev-guide sidebar confirms canonical landing slug `/docs/marketing/marketing-cloud/guide/choose-programming-lang.html` (replaces the deprecated `Personalization-Programming-Languages.html`). The sidebar also lists ~25 SSJS reference slugs (e.g., `ssjs_subscriberAdd.html`, `ssjs_WSProxy_perform.html`, `ssjs_error_handling.html`). Specific compile/payload/function-call limits live inside the page body and were not extracted from the captured shell. AMPscript runs server-side at send/render; SSJS executes server-side; both sandboxed. | https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/choose-programming-lang.html | TODO — canonical slug verified in dev-guide sidebar; specific per-language limits require browser navigation to the page body |

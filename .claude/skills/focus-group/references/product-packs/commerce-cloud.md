...
| **PCI scope** | TODO — `compliance.salesforce.com/en/services/b2c-commerce` was not confirmed as a stable canonical URL in 3 attempts. Salesforce Commerce can keep the merchant out of cardholder-data PCI scope when the certified payment integration is used; custom hosted-payment-page implementations can put scope back. Confirm specific PCI DSS attestation level and any current changes with the customer's QSA against current Salesforce Trust / Compliance documentation | (canonical compliance page not confirmed) | TODO — canonical compliance page not confirmed |
...
| **OCAPI / SCAPI / Shopper API** | TODO — the canonical "B2C Commerce API" overview page was not confirmed in 3 attempts (developer guide URLs returned JS shells on direct fetch). OCAPI (legacy Open Commerce API) and SCAPI (newer Salesforce Commerce API / Shopper APIs) are distinct API surfaces with different scope and limits; verify current limits and deprecation timing against `developer.salesforce.com/docs/commerce/` before scoping integrations | (canonical commerce-API overview not confirmed) | TODO — canonical commerce-API overview not confirmed |
| **Tax — B2B vs. B2C** | TODO — `cc.b2c_tax_service.htm` returns 404 and B2C Commerce help article slugs (`cc.*` and `commerce.*` namespaces) were not located on help.salesforce.com in 3 attempts; B2C Commerce help has migrated under `developer.salesforce.com/docs/commerce/`. Tax behaves differently in B2B (often Avalara/Vertex integration required) and B2C (built-in service for standard scenarios) — verify customer's tax scenarios against the current developer-docs tax page | (slug not located) | TODO — slug not located after 3 attempts |
...
| **Commerce + Agentforce / shopper agent** | TODO — a commerce-specific Agentforce capabilities page (e.g., shopper-agent on B2C Commerce) was not located on a stable canonical URL in 3 attempts. The general Agentforce product page (https://www.salesforce.com/agentforce/) does not commit to a specific commerce SKU. Confirm currently shipping Agentforce-for-commerce capabilities vs. roadmap from current release notes; do not position futures as currently available | (commerce-specific Agentforce page not located) | TODO — commerce-specific Agentforce page not located |
| **Internationalization / multi-site / multi-currency** | TODO — `cc.b2c_internationalization.htm` returns 404; B2C Commerce internationalization help has moved to `developer.salesforce.com/docs/commerce/`. The exact site-count / currency-count limits per realm/instance were not extracted from a canonical Salesforce-published page in 3 attempts — verify before any panel claim about multi-site/multi-currency caps | (slug not located) | TODO — slug not located after 3 attempts |

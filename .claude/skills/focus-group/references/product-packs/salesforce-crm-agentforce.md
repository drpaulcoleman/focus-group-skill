...
| **Agentforce — what counts as a conversation** | Salesforce's public pricing page does not formally define a conversation unit; the rate-card detail lives in the Agentforce Conversation rate-card PDF and the per-edition contract — TODO: pull the current rate-card PDF from https://www.salesforce.com/agentforce/rates/ and quote the conversation definition verbatim before any panel claim about session length, turns, or unresolved-handoff billing | https://www.salesforce.com/agentforce/rates/ | TODO — fetch current rate-card PDF. Re-attempted with real Chrome 2026-05-22 — still blocked. |
| **Agentforce — sharing model** | Agent actions inherit the running agent user's sharing rules and FLS — the agent user's permission set determines what the agent can see and do; this is a load-bearing fact for InfoSec. Re-attempted with real Chrome 2026-05-22 against `sf.agentforce_agent_user_permissions.htm`; help.salesforce.com SPA returned 461KB shell but article body not injected during headless capture, so slug validity remains unconfirmed. | https://help.salesforce.com/s/articleView?id=sf.agentforce_agent_user_permissions.htm | TODO — help SPA body not rendered in capture; verify slug and phrasing in a real browser |
| **Data Cloud dependency** | Required when the agent must reason over unstructured data, cross-cloud data, or data outside the Salesforce object model; not required for in-CRM-only agents — confirm against the current Agentforce decision guide before quoting. Re-attempted with real Chrome 2026-05-22; architect.salesforce.com is LWR-rendered and returns SPA shell (209KB) without decision-guide list body in headless capture. | https://architect.salesforce.com/decision-guides | TODO — decision-guide list body not rendered; pick current Agentforce guide via in-browser navigation |
| **Apex governor — SOQL queries per transaction** | "Total number of SOQL queries issued: 100 (synchronous limit) / 200 (asynchronous limit)." Cumulative cross-namespace limit: 1,100 SOQL queries per transaction. Cross-referenced from the verified row in [sales-cloud.md](sales-cloud.md). The previously-cited `apexref.meta` path is incorrect — governor limits live under `apexcode.meta`. | https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_gov_limits.htm | verified 2026-05-22 (cross-referenced from sales-cloud pack) |
| **Apex governor — DML rows per transaction** | "Total number of DML statements issued: 150 / 150 (sync / async)." "Total number of records processed as a result of DML statements, Approval.process, or database.emptyRecycleBin: 10,000 / 10,000." Cumulative cross-namespace DML statement limit: 1,650. Cross-referenced from verified row in [sales-cloud.md](sales-cloud.md). | https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_gov_limits.htm | verified 2026-05-22 (cross-referenced from sales-cloud pack) |
| **Bulk API 2.0 — daily request limit** | "Maximum number of records uploaded per 24-hour rolling period: 150,000,000" (Bulk API 1.0 and 2.0 share the same 150M record cap). Bulk API 2.0 ingest job max file size: 150 MB per job. Bulk API 2.0 daily query jobs limit: 10,000 per 24-hour rolling window. Cross-referenced from verified row in [sales-cloud.md](sales-cloud.md). | https://developer.salesforce.com/docs/atlas.en-us.salesforce_app_limits_cheatsheet.meta/salesforce_app_limits_cheatsheet/salesforce_app_limits_platform_bulkapi.htm | verified 2026-05-22 (cross-referenced from sales-cloud pack) |
...
| **Trust Layer — masking surface** | The Trust Layer's data masking replaces sensitive content with non-identifiable tokens before prompts reach the LLM; Salesforce's public framing covers PII and proprietary business data without enumerating each masked entity type on the public page. Re-attempted with real Chrome 2026-05-22; trusted-ai marketing page returned but the configurable masking-policy help article (which would enumerate default-vs-opt-in entity types) lives on help.salesforce.com SPA where article body is not rendered in headless capture. | https://www.salesforce.com/artificial-intelligence/trusted-ai/ | TODO — masking-policy entity list still requires in-browser navigation to the configurable help article |

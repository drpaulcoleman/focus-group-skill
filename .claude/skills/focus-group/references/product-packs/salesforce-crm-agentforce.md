...
|-------|------|-------------------|---------------|
| **Trust Layer — model providers** | Zero data retention is the default for Salesforce-supported model providers reached via the Trust Layer; bring-your-own-model arrangements have different retention guarantees per the customer's own contract with the provider | https://help.salesforce.com/s/articleView?id=sf.einstein_trust_layer.htm | TODO — verify slug |
| **Agentforce — pricing model** | Consumption-priced on conversations (not seats); high-volume customer-facing deployments must size the conversation envelope before quoting | https://www.salesforce.com/agentforce/pricing/ | TODO — verify URL |
| **Agentforce — sharing model** | Agent actions inherit the running user's sharing rules and FLS — the agent user's permission set determines what the agent can see and do | https://help.salesforce.com/s/articleView?id=sf.agentforce_agent_user_permissions.htm | TODO — verify slug |
| **Data Cloud dependency** | Required when the agent must reason over unstructured data, cross-cloud data, or data outside the Salesforce object model; not required for in-CRM-only agents | https://architect.salesforce.com/decision-guides | TODO — pick current decision guide |
...
| **Agentforce — feature GA status** | TODO — confirm GA vs. pilot vs. beta status of each Agentforce feature the panel discusses against the most recent release-notes article (Spring '26 / Summer '26 etc.) | https://help.salesforce.com/s/articleView?id=release-notes.rn_overview.htm | TODO — verify against current release |
| **Government Cloud Plus — feature parity** | TODO — confirm which Agentforce features are available in Government Cloud Plus (the set is materially smaller than commercial cloud and changes per release) | https://compliance.salesforce.com/en/services/government-cloud-plus | TODO |
| **Trust Layer — masking surface** | TODO — list which entity types are masked by default vs. opt-in, since this is a frequent customer question | https://help.salesforce.com/s/articleView?id=sf.einstein_trust_layer_data_masking.htm | TODO — verify slug |

...
| **CRM Analytics (formerly Tableau CRM / Einstein Analytics)** | A separate product from Tableau / Tableau Cloud, with a different data model (datasets, dashboards, lenses, SAQL) — do not conflate. Slug `sf.bi_overview.htm` returned the Salesforce Help wrapper page on real-Chrome render 2026-05-22 (heavy SPA shell, content not surfaced); no canonical slug confirmed. | https://help.salesforce.com/s/articleView?id=sf.bi_overview.htm | TODO — Salesforce Help SPA returns shell under headless Chrome; verify via PDF Analytics Library or Trailhead |
| **Row-level security at scale** | Four RLS approaches: (1) manual user filter — per-workbook, high-maintenance; (2) dynamic user filter using USERNAME() + security column — author must lock permissions; (3) data policies on virtual connections — needs Data Management license, server-enforced, recommended; (4) RLS in the source DB — doesn't work with extracts. | https://help.tableau.com/current/server/en-us/rls_options_overview.htm | verified 2026-05-22 |
| **Extract refresh schedules and live-query performance** | Tableau Cloud enforces a non-configurable 7200-sec (120-min) timeout per extract refresh; jobs exceeding it are canceled. Frequencies: hourly / daily (2/4/6/8/12-hr or once) / weekly / monthly. Same-workbook refreshes run serially; different workbooks run in parallel unless start times staggered. Bridge required for on-prem sources. | https://help.tableau.com/current/online/en-us/schedule_add.htm | verified 2026-05-22 |
| **Tableau + Data Cloud (Zero Copy or query federation)** | Salesforce Data Cloud connector shipped with Tableau Desktop 2023.2 (Desktop/Cloud/Server/Prep). Replaces older CDP connector (deprecated Oct 2023, removed Desktop 2024.3). Server needs extra config + JDBC v18+. As of Oct 14, 2025, Data Cloud rebranded to "Data 360"; both names appear in docs. | https://help.tableau.com/current/pro/desktop/en-us/examples_data_cloud.htm | verified 2026-05-22 |
| **Embedded analytics — licensing model** | Embedded Analytics is a distinct sales motion ("Contact sales for evaluation") — no public list price; do not quote standard Tableau Cloud pricing. Capabilities: REST API for user/content/permissions, trusted authentication (your app provisions identity), domain allowlist, SSO, Pulse metrics layer, Embedding Playground. | https://www.tableau.com/embedded-analytics | verified 2026-05-22 |
| **Migration from Power BI / Looker / ThoughtSpot / Sigma** | Migrating dashboards is real work; semantic-model translation rarely round-trips cleanly. No canonical Salesforce-published migration playbook page identified at slug `/partners` — partner-led migrations dominate. Scope honestly: dashboard recreation, semantic-layer rebuild, training, and data-source connection rework are typical line items. | https://www.tableau.com/partners | TODO — no canonical migration playbook on tableau.com; partner pages vary; verify per-deal |
| **Governance — published data sources and content certification** | Tableau Blueprint defines two governance domains: Data Governance (source mgmt, quality, security, monitoring) and Content Governance (content mgmt, authoring, certification, promotion). Without standards and policies, expect duplicate data sources and user confusion. Older `server/governance_overview.htm` slug now 404s; canonical = Blueprint. | https://help.tableau.com/current/blueprint/en-us/bp_governance_in_tableau.htm | verified 2026-05-22 |

...
| **CPQ — bundle, configuration, and price-rule engine** | TODO — `sf.cpq_overview.htm` returns 404; canonical CPQ-overview slug on help.salesforce.com was not located in 3 attempts (`sales.cpq_overview`, `sf.cpq_intro`, etc. all 404). Pricing logic lives in CPQ-specific objects but the specific object names and rule-engine specifics need verification against the current legacy-CPQ documentation set before any panel claim | (slug not located) | TODO — slug not located after 3 attempts |
| **Amendments, co-terming, ramps, renewals** | TODO — `sf.cpq_subscription_management.htm`, `sales.cpq_amendments.htm`, `sales.cpq_amend_renewal_concept.htm`, `sales.cpq_renewals.htm` all 404. Canonical help slug for CPQ amendments/co-terming/renewals was not located in 3 attempts; this is the topic where CPQ projects most often slip, so any panel claim should be sourced from current CPQ release notes or the Revenue Cloud Advanced lifecycle docs | (slug not located) | TODO — slug not located after 3 attempts |
| **Revenue recognition — ASC 606 / IFRS 15** | TODO — `sf.blng_revenue_recognition.htm` returns 404. The current Salesforce Billing overview (https://help.salesforce.com/s/articleView?id=sales.blng_overview.htm&type=5) and "Bookings, Billings, Cash, and Revenue" review (https://help.salesforce.com/s/articleView?id=sales.blng_revenue_process_review_intro.htm&type=5) confirm Salesforce Billing has a Revenue Recognition Reporting concept, but a single canonical "ASC 606 / IFRS 15" article slug was not located in 3 attempts — leave the standard-naming claim unsourced until the current article is confirmed | (slug not located) | TODO — slug not located after 3 attempts |
...
| **API limits — quote and order operations** | TODO — current quote/order API limits and bulk-operation patterns were not located on a stable canonical URL in 3 attempts (legacy CPQ developer guide URLs return JS shells / 404s on direct fetch). Verify against the current `developer.salesforce.com` Revenue Cloud / CPQ developer guides before any panel claim about throttles or bulk patterns | (canonical limits page not located) | TODO — canonical limits page not located |
| **Data model — Order, OrderItem, Asset, Subscription** | TODO — `sf.order_management.htm`, `sales.order_management.htm`, `service.order_management.htm`, `ind.order_management.htm`, `commerce.order_management.htm` all 404. The Salesforce Order Management product page (https://www.salesforce.com/commerce/order-management/) confirms Order Management is a distinct product, but the canonical help-doc slug for the Order/OrderItem/Asset/Subscription lifecycle data model was not located in 3 attempts — verify against the current Salesforce help / object reference before any panel claim about lifecycle states | (slug not located) | TODO — slug not located after 3 attempts |
